...
import sys
from rag_pipeline import retrieve, DEFAULT_K
...
@tool
def retrieve_tool(query: str, k: int = DEFAULT_K):
    """
    Search the DME LCD knowledge base for chunks relevant to the query.
    Use this before answering any DME coverage or prior-authorization
...
if __name__ == "__main__":
    ...
    result1 = model_with_tools.invoke(
        [HumanMessage(content="What are the coverage criteria for a power wheelchair under Medicare?")]
    )
    ...
    messages = [HumanMessage(content="What documentation is required for an oxygen concentrator LCD?")]
