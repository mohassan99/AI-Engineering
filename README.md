...
(LangGraph agent orchestration) in progress (B.1 done). Stage C (documented
prompt engineering) not yet started.
...
## Stage B — LangGraph agent orchestration (in progress)

**Design (locked):** a hand-built LangGraph `StateGraph` (Graph API, not
Functional API) with one orchestrator node bound to a single retrieval
tool — not a two-agent "orchestrator + specialist" split, and not the
higher-level `create_agent` abstraction, which would hide the routing logic
later stages need to log and debug. The tool wraps `retrieve()` only, not
`answer()`, so each turn costs exactly one Claude generation call (inside
the orchestrator), not two. Scope routing (in-scope vs. wrong-jurisdiction
vs. non-DME vs. non-traditional-Medicare vs. fully unrelated) lives in the
orchestrator's system prompt rather than a separate upstream classifier
node, to avoid doubling Claude calls for the common case.

**B.1 — framework setup & sanity check: done.** Before wiring in the real
retriever, a throwaway script (gitignored, not committed) confirmed the
basic building blocks work end-to-end against Claude Sonnet 5: a dummy
tool bound via `bind_tools()`, correctly triggered on an on-topic query and
correctly skipped on an off-topic one, wired into a full
`START → llm_call → tools_condition → tool_node → llm_call → END` loop
using LangGraph's prebuilt `ToolNode`/`tools_condition` utilities (kept as
a deliberate, locked design choice even where the current LangGraph
quickstart docs demonstrate a hand-rolled equivalent instead).
...
- **Windows terminal encoding.** Git Bash's default terminal encoding
  ...
  `UnicodeEncodeError` on otherwise-correct output. Fixed the same way
  twice now (once in Stage A's `generate()`, once in Stage B's sanity
  check) with `sys.stdout.reconfigure(encoding="utf-8", errors="replace")`
  at each script's entry point — a real, recurring cross-platform
  constraint of this dev environment, not a one-off.

**Next:** B.2 — replace the dummy tool with the real `retrieve()` from
`rag_pipeline.py`.
...
- **Stage B:** LangGraph agent orchestration — an orchestrator agent that
  routes between the RAG retriever and direct-answer paths. B.1 done; see
  above.
